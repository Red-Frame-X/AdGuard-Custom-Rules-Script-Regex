...
- [`Forced Updates via Crosh (Chrome OS Developer Shell).md`](Forced%20Updates%20via%20Crosh%20%28Chrome%20OS%20Developer%20Shell%29.md)：ChromeOSの安全な手動更新手順と、更新エラー時のハードウェアリセットを含むトラブルシューティング
...
- [`Distributing Filters and UserScripts with GitHub Gist`](How%20to%20Use%20Gists.md)：GitHub Gistを使ったコンテンツブロックフィルタ／UserScriptの配布・更新方法とメタデータテンプレート
...
各文書では公式ドキュメントやプロジェクト自身の公開資料を優先します。公式資料だけで確認できない場合でも、Web検索によって信頼できる一次資料・公開ソース・Issues・複数の独立した情報源などから十分な裏付けを得られる内容は残します。
