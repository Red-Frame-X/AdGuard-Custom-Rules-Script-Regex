...
exact("| **Version** | 20260825 |", "| **Version** | 20260908 |", "version")
exact(
...
sub(r"\n---\n\n## ChromeOS Chrome テーマ\n.*?\n---\n\n## ChromeOS Chrome アプリ", "\n---\n\n## ChromeOS Chrome アプリ", "theme section")
exact(
...
exact("* **[uBlock Origin – Badware risks](https://github.com/uBlockOrigin/uAssets)**：Yuki2718氏がフィルタの監修に関わっています。", "* **[uBlock Origin – Badware risks](https://github.com/uBlockOrigin/uAssets)**：uBlock Originの公式uAssetsで管理される悪質サイト対策用フィルタ。個々のルールの作者・レビュー担当は公開履歴で確認します。", "uAssets attribution")
exact("* **[AdGuard Japanese filter Plus](https://github.com/Yuki2718/adblock2)**：Yuki2718氏自身がフィルタの監修をしています。", "* **[AdGuard Japanese filter Plus](https://github.com/Yuki2718/adblock2)**：Yuki2718氏の公開リポジトリでメンテナンスされている追加フィルタ。", "jpf attribution")
sub(
    r"AdGuard ブラウザ拡張機能 MV3対応版でスクリプトレットを含む高度なルールを使用する場合は、.*?\n\n\*\*カスタムフィルタで購読する場合\*\*",
...
exact("※ 実態はDNSブロッカーのため、ABP形式の構文（`||example.com^`）には対応していません。", "※ ダウンロードするブロックリストはhosts / domains-only系の単純な形式を使用します。`additionalHosts.txt`では`!`によるホワイトリスト、`*`ワイルドカード、`>`によるカスタムIP指定など独自の追加構文を利用できますが、Adblock Plus形式の`||example.com^` / `@@||example.com^`とは別の構文です。", "personalDNSfilter syntax")
exact("Android版Chromeなどで高精度なブロックを行うには必須です。暗号化通信を一時解析し、要素をブロックします。Personal CA証明書のインストールが必要です。", "HTTPSサイトや対応アプリの暗号化通信を内容まで検査して、ネットワークルール、要素隠蔽、スクリプトレット等を十分に適用するにはHTTPSフィルタリングが重要です。AdGuardは端末内で通信を復号・検査して再暗号化するため、ユーザーCA証明書のインストールが必要です。HTTPSフィルタリングを無効にしてもDNSフィルタリングや一部のネットワーク遮断まで全て無効になるわけではありませんが、暗号化された通信内容を利用する高度な処理は制限されます。", "HTTPS description")
sub(
...
sub(
    ...
    "Android 17 device admin",
)
exact("③ **MacroDroidを利用したVPN自動再接続タスクの作成**（最も効果的）", "③ **MacroDroidを利用したVPN自動再接続タスクの作成**（個人環境での復帰補助ワークアラウンド）", "MacroDroid claim")
exact("* WebViewの影響は v4.10 以降で修正されました。OSによるタスクキル問題は常時接続VPNの有効化やMacroDroidの導入で対処します。（[タスクキル対策ガイド](https://adguard.com/kb/ja/adguard-for-android/solving-problems/background-work/)）", "* WebView関連の既知問題は v4.10系で修正されています。バックグラウンド停止については、まずAdGuard公式のメーカー別タスクキル対策と常時接続VPNを確認します。MacroDroidは公式の必須手順ではなく、自分の環境で必要性と副作用を確認して使う補助策です。（[タスクキル対策ガイド](https://adguard.com/kb/ja/adguard-for-android/solving-problems/background-work/)）", "WebView conclusion")
exact("スマートフォンの「プライベートDNS」設定に特定のホスト名を入力するだけで、端末全体に表示される広告を非表示にすることができます。", "スマートフォンの「プライベートDNS」へ広告・トラッカー遮断を行うDNSサービスのホスト名を設定すると、DNSで識別できる対象ドメインへの接続を端末全体で遮断できます。同一ドメインから配信される広告、ページ内要素、URLパス単位の広告などはDNSだけでは除去できません。", "Private DNS scope")
sub(
    r"\* 入力欄に以下のホスト名を入力して保存します。現在はより安定している\*\*新バージョン\*\*の入力が推奨されています。.*?`dns\.adguard\.com`",
